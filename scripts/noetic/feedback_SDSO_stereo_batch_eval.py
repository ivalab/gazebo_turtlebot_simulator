...
VI_SDSO_PATH = os.path.join(os.environ["HOME"], "closedloop_ws/src/stereo_DSO/")
VI_SDSO_NODE = os.path.join(
    os.environ["HOME"], "closedloop_ws/devel/.private/stereo_dso_ros/lib/stereo_dso_ros/sdso_live"
)
METHOD_NAME = "SDSO"
            ...
            for sn, sname in enumerate(SeqNameList):
                ...
                for iteration in range(0, Num_Repeating):
                    ...
                    cmd_slam = (
                        ...
                        + " calib0="
                        + os.path.join(VI_SDSO_PATH, "calib/gazebo/cam0.txt")
                        + " calib1="
                        + os.path.join(VI_SDSO_PATH, "calib/gazebo/cam1.txt")
                        + " T_stereo="
                        + os.path.join(VI_SDSO_PATH, "calib/gazebo/T_C0C1.txt")
                        + " preset=0 mode=1 nomt=0 glog_loglevel=1 enable_closed_loop=1"
                        + " pointdensity="
                        + str(num_gf)
                        + " preload=0"
                        + " nogui=1"
