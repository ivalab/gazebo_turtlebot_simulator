        ...
        for vn, fv in enumerate(Fwd_Vel_List):
            for sn, sname in enumerate(SeqNameList):
                ...
                for iteration in range(0, Num_Repeating):
                    ...
                    cmd_vins = str(
                        "python call_vinsfusion.py -f " + str(num_gf) + " -i " + IMU_Type + " -l " + path_data_logging
                    )
                    # cmd_init = str("python init_VINS.py")
                    cmd_esti = str(
                        "roslaunch ../launch/gazebo_odom_conversion.launch"
                    ...
                    print(bcolors.OKGREEN + "Launching vins" + bcolors.ENDC)
                    subprocess.Popen(cmd_vins, shell=True)
                    # wait for VINS to stablize
                    time.sleep(SleepTime)
                    ...
                    print(bcolors.OKGREEN + "Launching Odometry Conversion" + bcolors.ENDC)
